...
def write():
    ...
    st.subheader(
        'by Jiapeng Zhu,  Yujun Shen,  Deli Zhao,  Bolei Zhou ([@GenForce\_](https://genforce.github.io/))')
    ...
    operation = st.selectbox(
        'Please choose the operation you want', list(idvNode), index=0)
...
def writePageNode(operation):
    ...
    if operation == '✨ Semantic Diffusion':
        Semantic_Diffusion()
...
def Semantic_Diffusion():
    ...
    if target_img and context_img:
        ...
        if revert:
            cropImg = cropHelper(target_img, context_img, crop_size, crop_size)
        else:
            cropImg = cropHelper(context_img, target_img, crop_size, crop_size)

        if A1.button('🚀Comfirm and start generate'):
            if revert:
                with st.spinner('Diffusing the images...⏳'):
                    result_img = idvTool.diffuse(
                        target_img_cv, context_img_cv, crop_size)
            else:
                with st.spinner('Diffusing the images...⏳'):
                    result_img = idvTool.diffuse(
                        context_img_cv, target_img_cv, crop_size)

        if result_img is None:
            A2.image(cropImg, use_column_width=True)
        else:
...
def Interpolation():
    ...
    src_file = a1.file_uploader(
        label="First image!", type=['jpg', 'png', 'jpeg'])
    tar_file = b1.file_uploader(
        label="Sencond image!", type=['jpg', 'png', 'jpeg'])

    if src_file:
        src_file = Image.open(src_file)
        src_file.save(os.path.abspath(
            'img/indomain_output/interpolate/context_temp/temp.png'), 'PNG')
        src_file_path = os.path.abspath(
            'img/indomain_output/interpolate/context_temp/temp.png')
        a2.image(src_file, 'First Photo', use_column_width=True)

    if tar_file:
        tar_file = Image.open(tar_file)
        tar_file.save(os.path.abspath(
            'img/indomain_output/interpolate/target_temp/temp.png'), 'PNG')
        tar_file_path = os.path.abspath(
            'img/indomain_output/interpolate/target_temp/temp.png')
    ...
    if st.button('🚀Click this button to generate'):
        ...
        for i, image in enumerate(result):
            ...
            image = cv2.imwrite(
                'img/indomain_output/interpolate/result/' + str(i) + '.png', image)
            img = Image.open(
                'img/indomain_output/interpolate/result/' + str(i) + '.png')
            if i == 0:
                for j in range(100):
        ...
        for i in range(100):
            result_img.append(result_img_invert[0])
        for image in result_img_invert:
            result_img.append(image)
        result_img[0].save('img/indomain_output/interpolate/result/gif.gif', save_all=True,
                           append_images=result_img[1:], optimize=False, duration=5, loop=0)
...
def Manipulation():
    ...
    uploadImg = A1.file_uploader(label="Upload your image!", type=[
                                 'jpg', 'png', 'jpeg'], accept_multiple_files=False)

    if uploadImg:
        src_file_path = os.path.abspath(
            'img/indomain_output/manipulation/temp/temp.png')
...
def uploadImg(column, str):
    ...
    img_file = column.file_uploader(
        label=f"Upload the {str} image!", type=['jpg', 'png', 'jpeg'])
...
def cropHelper(context_img, target_img, crop_width, crop_height):
    ...
    crop_img = context_img.crop(((img_width - crop_width) // 2,
                                 ...
                                 (img_height + crop_height) // 2))
    target_img.paste(crop_img, ((img_width - crop_width) //
                     2, (img_height - crop_height)//2))
    return target_img
